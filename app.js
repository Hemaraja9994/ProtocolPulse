...
function handleAuthClick() {
    tokenClient.callback = async (resp) => { if (resp.error) return; document.getElementById('driveBtn').innerText = "Cloud Synced"; saveToDrive(); };
...
async function executeAnalysis() {
    ...
    const prompt = `Act as an Indian Institutional Ethics Committee Assistant (ICMR 2017 & NDCTR 2019). 
    REVIEW TYPE: ${currentCategory.toUpperCase()}. 
    TEXT: ${aggregatedText.substring(0, 15000)}
    
    Output ONLY pure JSON format with following keys: consensus, chairperson, secretary, lawyer, clinician, layperson.
    Each persona MUST have "analysis" (string), "score" (0-100), and "checks" (array of {item, status, note}).
    Statuses: success, modify, scrutinize.`;
    ...
    try {
        ...
        // ROBUST JSON PARSING: Handles markdown backticks and extra AI commentary
        ...
        console.error(e);
        alert("AI Session Failed. Check Key/Connection."); 
    }
    finally { btn.disabled = false; btn.innerText = "Run Targeted Audit"; }
}

function renderResult() {
    const data = auditData[currentPersona];
    document.getElementById('viewLabel').innerText = currentPersona.toUpperCase();
    document.getElementById('roleTitle').innerText = currentPersona.toUpperCase() + " Audit View";
    ...
    data.checks.forEach(c => {
        ...
        html += `<div class="${type} p-4 rounded-xl shadow-sm mb-2">
...
function exportPDF() {
    ...
    doc.setTextColor(255); doc.setFontSize(20); doc.text("EthicShield Formal Review Report", 15, 25);
    doc.setFontSize(8); doc.text("Conceptualized and Academic Designed by Mr. Hemaraja Nayaka.S", 15, 35);
    doc.autoTable({ startY: 60, head: [['EC Requirement', 'Status', 'Observation']], body: auditData.consensus.checks.map(c => [c.item, c.status.toUpperCase(), c.note]), theme: 'grid' });
    doc.save("Ethics_Scrutiny_Report.pdf");
}
    ...
    const doc = new Document({
        sections: [{ children: [
            ...
            new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [
                new TableRow({ children: [ new TableCell({ children: [new Paragraph("Item")] }), new TableCell({ children: [new Paragraph("Status")] }), new TableCell({ children: [new Paragraph("Note")] }) ]}),
...
async function saveToDrive() {
    ...
    form.append('metadata', new Blob([JSON.stringify({ name: `Ethics_Audit_${Date.now()}.json`, mimeType: 'application/json' })], { type: 'application/json' }));
